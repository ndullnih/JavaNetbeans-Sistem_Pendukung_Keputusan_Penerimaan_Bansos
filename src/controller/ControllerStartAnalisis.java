...
import ahpprocess.ProcessAHP;
import dao.DAOBobotKriteria;
import dao.DAOBobotSubKriteria;
import dao.DAOKriteria;
import dao.DAONilaiBobot;
import dao.DAOSubKriteria;
import dao.InterfaceBobotKriteria;
import dao.InterfaceBobotSubKriteria;
import dao.InterfaceKriteria;
import dao.InterfaceNilaiBobot;
import dao.InterfaceSubKriteria;
import model.ModelBobotKriteria;
...
public class ControllerStartAnalisis {
    ...
    private void ProcessKaryawan(){
        List<ModelNilaiSupplier> list = in_nilaibobot.getNilaiKaryawan();
        
        for(int i=0; i<list.size(); i++){
            ModelNilaiSupplier k = list.get(i);
            in_nilaibobot.insertNilaiKaryawan(k);
        }
        JOptionPane.showMessageDialog(frm, "Proses perhitungan selesai.","Info",1);
    }
}
